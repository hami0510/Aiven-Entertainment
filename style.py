...
def sidebar_brand():
    ...
    today_html = (
        '<div style="margin:12px 4px 4px 4px; padding:11px 13px; border:1px solid #E4E4E4; '
    ...
    if today_items:
        for icon, title in today_items[:3]:
            ...
            today_html += f'<div style="font-size:12px; color:#0D0D0D; margin-bottom:3px;">{icon} {short}</div>'
    ...
    # ---- 3. 빠른 검색 ----
    st.sidebar.markdown(
        '<div style="margin:10px 4px 4px 4px; font-size:11px; font-weight:800; color:#767676; '
    ...
    # ---- SNS 계정 ----
    html = '<div style="padding: 12px 4px 0 4px;">'
    for account in SIDEBAR_ACCOUNTS:
        html += (
            '<div style="margin-bottom:16px;">'
            f'<div style="font-size:12px; font-weight:800; letter-spacing:0.4px; color:#767676; '
            f'text-transform:uppercase; margin-bottom:6px;">{account["name"]}</div>'
            '<div style="display:flex; gap:16px;">'
        )
        for link in account["links"]:
            html += (
                f'<a href="{link["url"]}" target="_blank" title="{link["label"]}" '
                'style="text-decoration:none; font-size:24px; line-height:1;">'
                f'{link["icon"]}</a>'
            )
        html += '</div></div>'
    html += '</div>'
